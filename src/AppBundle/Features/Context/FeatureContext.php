...
class FeatureContext extends AbstractContext
{
    ...
    {
        $script = sprintf('$("#%s").fineUploader("addFiles", { name: "%s", blob: new Blob(["%s"]) });', $container, $fileName, 'abc');
    ...
    public function noFileIsSavedInto($fileName)
    {
        ...
        if (is_file($root . '/' . $fileName)) {
            throw new \InvalidArgumentException(sprintf('File %s found inside %s, but it should not exist', $fileName, $root));
        }
    }
    ...
    {
        $this->getSession()->wait(10000, '($(".tt-highlight").length !== 0) || ($(".tt-dataset .empty-list").length !== 0)');
    }
