...
class RoutingExtension extends \Twig_Extension
{
    ...
    public function getFunctions()
    {
        return [
            new \Twig_SimpleFunction(
                'granted_path', [$this, 'getPath'], [
                'is_safe_callback' => [$this, 'isUrlGenerationSafe'],
            ]
            ),
            new \Twig_SimpleFunction(
                'format_create_link', [$this, 'formatCreateLink'], [
                'is_safe' => ['html'],
            ]
            ),
            new \Twig_SimpleFunction(
                'format_edit_link', [$this, 'formatEditLink'], [
                'is_safe' => ['html'],
            ]
            ),
            new \Twig_SimpleFunction(
                'format_delete_link', [$this, 'formatDeleteLink'], [
                'is_safe' => ['html'],
            ]
            ),
            new \Twig_SimpleFunction(
                'format_view_link', [$this, 'formatViewLink'], [
                'is_safe' => ['html'],
            ]
            ),
            new \Twig_SimpleFunction(
                'format_edit_icon', [$this, 'formatEditIcon'], [
                'is_safe' => ['html'],
            ]
            ),
            new \Twig_SimpleFunction(
                'format_view_icon', [$this, 'formatViewIcon'], [
                'is_safe' => ['html'],
            ]
            ),
            new \Twig_SimpleFunction(
                'format_delete_icon', [$this, 'formatDeleteIcon'], [
                'is_safe' => ['html'],
            ]
            ),
            new \Twig_SimpleFunction(
                'format_action_icon', [$this, 'formatActionIcon'], [
                'is_safe' => ['html'],
            ]
            ),
            new \Twig_SimpleFunction(
                'format_action_link', [$this, 'formatActionLink'], [
                'is_safe' => ['html'],
            ]
            ),
        ];
    ...
    public function formatActionLink($resource, $action, $text, $object = null, array $parameters = [])
    {
        ...
        if ($object) {
            if ($this->authorizationChecker->isGranted($role, $object) && $enabled) {
                ...
                $url = $this->generator->generate(
                    $routeName,
                    array_merge($routeParameters, ['id' => $object->getId()])
                );
        ...
        return $showDisabled ? $this->formatLink(
            $action,
            '#',
            '',
            $text,
            array_merge(['disabled' => true], $parameters)
        ) : '';
    }
    ...
    {
        return sprintf(
            '<a class="%s %s" href="%s" title="%s" %s>%s</a>',
            $action,
